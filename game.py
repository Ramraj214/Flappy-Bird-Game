import random #for generating random numbrs
...
def getRandomPipe():
    """generating two pipes(one bottom and one top rotated) for blitting on the screen
    """
    pipeHeight=GAME_SPRITES['pipe'][0].get_height()
    offSet=SCREENHEIGHT/3
    y2=offSet+random.randrange(0,int(SCREENHEIGHT-GAME_SPRITES['base'].get_height()-1.2*offSet))
    pipeX=SCREENWIDTH+13
...
if __name__ == "__main__":
    #This will be the main point from from where our game will start
    pygame.init() #initialize all pygame modules
    FPSCLOCK=pygame.time.Clock()
    pygame.display.set_caption("Flappy Bird By Ramraj07")
    ...
    #game sounds
    GAME_SOUNDS['die']=pygame.mixer.Sound('Audio/download.wav')
    GAME_SOUNDS['hit']=pygame.mixer.Sound('Audio/download (1).wav')
    GAME_SOUNDS['point']=pygame.mixer.Sound('Audio/download (2).wav')
    GAME_SOUNDS['swoosh']=pygame.mixer.Sound('Audio/download (3).wav')
    GAME_SOUNDS['wing']=pygame.mixer.Sound('Audio/download (4).wav')

    GAME_SPRITES['background']=pygame.image.load(BACKGROUND).convert()
    GAME_SPRITES['player']=pygame.image.load(PLAYER).convert_alpha()

    while True:
        welcomeScreen() #Show welcome screen until player presses the button
